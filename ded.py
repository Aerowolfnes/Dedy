...
def idteman():
	if log != 1:
		tampil('\rh[*]Login dulu harap masuk...')
		login()
		if log == 0:
			keluar()
...
def lanjutT():
	global fid_bteman
	if len(fid_bteman) != 0:
		i = inputD('[?]Riset Hasil Teman/lanjutkan (r/l)',['R','L'])
		if i.upper() == 'L':
...
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		i = inputD('[?]Riset Hasil Group/lanjutkan (r/l)',['R','L'])
		if i.upper() == 'L':
...
\rk#########################################
           \rb*Hack FACEBOOK*\rk              
\rhBY\rp Dedy Rohady\rk
\rhFB\rp https://m.facebook.com/aerowolf.id \rk
\rhGitHub\rp https://github.com/Aerowolfnes \rk ###########################################''')
	tampil('''\rk%s\n\rc1 \rhHack Dari group\n\rc2 \rhHack teman\n\rc3 \rmKELUAR\n\rk%s'''%('#'*20,'#'*20))
